-- =============================================================================
-- 20260827_000007_crear_cita_y_cobrar_estado.sql
-- Agrega p_estado a crear_cita_y_cobrar_atomico (pendiente | pagado).
-- "Agendar" → cobro pendiente; "Agendar y Pagar" → cobro pagado.
-- Idempotente.
-- =============================================================================
...
DROP FUNCTION IF EXISTS public.crear_cita_y_cobrar_atomico(
  bigint, bigint, bigint[], date, time, time, numeric, text, text, date, text, text
);
DROP FUNCTION IF EXISTS public.crear_cita_y_cobrar_atomico(
  bigint, bigint, bigint, date, time, time, numeric, text, text, date, text
);
...
COMMENT ON FUNCTION public.crear_cita_y_cobrar_atomico(
  bigint, bigint, bigint[], date, time, time, numeric, text, text, date, text, text
) IS
  'Crea agenda + cobro atómico. p_estado: pendiente (Agendar) o pagado (Agendar y Pagar).';
