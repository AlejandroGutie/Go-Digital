-- Crear cobro con estado inicial pagado o pendiente (CobrosPage)
...
BEGIN
  ...
  -- Estado lo define el botón (Pagar / Pendiente); no es campo del formulario
...
GRANT EXECUTE ON FUNCTION public.create_cobro_atomico(
  bigint, bigint, bigint, bigint[], numeric, text, text, date, text
) TO authenticated;

-- Wrapper legacy (una tarifa): sigue creando como pagado por defecto
CREATE OR REPLACE FUNCTION public.create_cobro_atomico(
  p_id_agenda bigint,
  p_id_profesional bigint,
  p_id_mascota bigint,
